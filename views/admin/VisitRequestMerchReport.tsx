...
import { VisitRequest, Customer, Merch } from '../../types';
        ...
        <div className="bg-surface dark:bg-dark-surface rounded-lg shadow-md border border-border dark:border-dark-border p-4 flex flex-col space-y-3">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-sm text-text-secondary dark:text-dark-text-secondary">{request.merch?.name}</p>
...
const VisitRequestMerchReport: React.FC = () => {
    const { t, permissions, showNotification } = useAppContext();
    const pagePermissions = permissions['Visit Request Merch Report'];
    ...
    const fetchDropdownData = useCallback(async () => {
        try {
            ...
            const [{ data: merchsData, error: merchsError }, { data: customersData, error: customersError }] = await Promise.all([merchsPromise, customersPromise]);
            if (merchsError || customersError) throw merchsError || customersError;
    ...
    const fetchReports = useCallback(async (page: number, currentFilters: typeof filters) => {
        ...
        try {
            ...
            if (currentFilters.entityId) {
                if (currentFilters.filterType === 'merchandiser') query = query.eq('merch_id', currentFilters.entityId);
    ...
    return (
        <div className="space-y-6">
            <h1 className="text-3xl font-bold">{t('visitRequestMerchReport.title')}</h1>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                     ...
                     <div>
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => handleFilterChange('filterType', 'merchandiser')} className={`flex-1 p-2 rounded-md text-sm ${filters.filterType === 'merchandiser' ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'}`}>{t('visitRequestMerchReport.filters.merchandiser')}</button>
                     ...
                     <div>
                         ...
                         <select value={filters.entityId} onChange={e => handleFilterChange('entityId', e.target.value)} className="w-full p-2 bg-surface dark:bg-dark-surface border border-border dark:border-dark-border rounded-md text-sm">
                             <option value="">{filters.filterType === 'merchandiser' ? t('visitRequestMerchReport.filters.selectMerchandiser') : t('visitRequestReport.filters.selectCustomer')} ({t('visitRequestReport.filters.all')})</option>
                             {(filters.filterType === 'merchandiser' ? merchs : customers).map(entity => <option key={entity.id} value={entity.id}>{entity.name}</option>)}
                         </select>
                    </div>
                     <div>
            ...
            {loading ? <div className="text-center p-8"><Loader2 className="h-8 w-8 animate-spin mx-auto text-primary" /></div> :
             requests.length === 0 ? <p className="text-center py-8 text-text-secondary">{t('visitRequestMerchReport.noResults')}</p> :
             (
                <>
...
export default VisitRequestMerchReport;