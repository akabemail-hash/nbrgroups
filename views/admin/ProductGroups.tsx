...
const ProductGroupModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    onSave: (group: Partial<ProductGroup>) => Promise<void>;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" aria-modal="true" role="dialog">
            <div className="bg-surface dark:bg-dark-surface rounded-lg shadow-xl w-full max-w-lg">
                <div className="flex justify-between items-center p-4 border-b border-border dark:border-dark-border">
                    <h2 className="text-xl font-bold">{group?.id ? t('productGroups.form.title.edit') : t('productGroups.form.title.add')}</h2>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-1">{t('productGroups.name')}</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-1">{t('productGroups.description')}</label>
                            <textarea name="description" value={formData.description || ''} onChange={handleChange} rows={3} className="w-full py-2 px-3 bg-surface dark:bg-dark-surface border border-border dark:border-dark-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary dark:focus:ring-dark-primary" />
                        </div>
                    </div>
...
const ProductGroups: React.FC = () => {
    ...
    const pagePermissions = permissions['Product Groups'];
    const [groups, setGroups] = useState<ProductGroup[]>([]);
    const [loading, setLoading] = useState(true);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingGroup, setEditingGroup] = useState<Partial<ProductGroup> | null>(null);
    ...
    const fetchGroups = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const { data, error } = await supabase.from('product_groups').select('*').order('name', { ascending: true });
            if (error) throw error;
            if (data) setGroups(data);
        } catch (error: any) {
            console.error("Error fetching product groups:", error);
            showNotification(`Failed to fetch product groups: ${error.message}`, 'error');
        } finally {
            setLoading(false);
        }
    ...
    const handleDeleteConfirm = (group: ProductGroup) => {
        setDeletingGroup(group);
    };
    
    const handleSave = async (groupData: Partial<ProductGroup>) => {
        const isNew = !groupData.id;
        try {
            const dataToSave = { ...groupData, updated_at: new Date().toISOString() };
            const { error } = await supabase.from('product_groups').upsert(dataToSave);
            if (error) throw new Error(error.message);
            
            showNotification(isNew ? t('notification.productGroup.added') : t('notification.productGroup.updated'));
            setIsModalOpen(false);
            fetchGroups();

        } catch (error: any) {
            console.error("Save product group error:", error);
            if (error.message && error.message.includes('product_groups_name_key')) {
                throw new Error(t('notification.productGroup.saveErrorUnique'));
            }
            throw error;
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            const { error } = await supabase.from('product_groups').delete().eq('id', deletingGroup.id);
            if (error) throw error;
            
            showNotification(t('notification.productGroup.deleted'));
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h1 className="text-3xl font-bold text-text-primary dark:text-dark-text-primary">{t('productGroups.title')}</h1>
                {pagePermissions?.can_create && <button onClick={handleAdd} className="flex items-center px-4 py-2 bg-primary dark:bg-dark-primary text-white rounded-md hover:bg-secondary dark:hover:bg-dark-secondary transition-colors"><Plus className="h-5 w-5 mr-2" />{t('productGroups.addGroup')}</button>}
            </div>
                ...
                <div className="bg-surface dark:bg-dark-surface rounded-lg shadow-md border border-border dark:border-dark-border overflow-x-auto">
                    <table className="min-w-full divide-y divide-border dark:divide-dark-border">
                        <thead className="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-text-secondary dark:text-dark-text-secondary uppercase tracking-wider">{t('productGroups.name')}</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-text-secondary dark:text-dark-text-secondary uppercase tracking-wider">{t('productGroups.description')}</th>
                        ...
                        <tbody className="bg-surface dark:bg-dark-surface divide-y divide-border dark:divide-dark-border">
                            {groups.map(group => (
                                <tr key={group.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary dark:text-dark-text-secondary">{group.description}</td>
            ...
            <ProductGroupModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSave} group={editingGroup} />
                 ...
                 <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" aria-modal="true" role="dialog">
                    <div className="bg-surface dark:bg-dark-surface rounded-lg shadow-xl w-full max-w-sm">
                        <div className="p-6 text-center">
                            ...
                            <h3 className="text-lg font-bold mb-2">{t('form.delete')} Product Group</h3>
                            <p className="text-sm text-text-secondary dark:text-dark-text-secondary mb-6">{t('productGroups.form.confirmDelete')}</p>
