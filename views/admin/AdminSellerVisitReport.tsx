...
const ReportCard: React.FC<{ visit: SellerVisit, onImageClick: (url: string) => void }> = ({ visit, onImageClick }) => {
    const { t } = useAppContext();
    return (
        <div className="bg-surface dark:bg-dark-surface rounded-lg shadow-md border border-border dark:border-dark-border p-4 flex flex-col space-y-4">
            <div className="flex justify-between items-start border-b border-border dark:border-dark-border pb-3">
                <div>
                    ...
                    <p className="text-sm font-medium text-primary dark:text-dark-primary">{visit.seller?.name}</p>
...
const AdminSellerVisitReport: React.FC = () => {
    const { t, permissions, showNotification } = useAppContext();
    const pagePermissions = permissions['Admin Seller Visit Report'];

    const [visits, setVisits] = useState<SellerVisit[]>([]);
    ...
    const fetchReports = useCallback(async () => {
        ...
        try {
            ...
            if (sellers.length === 0 || customers.length === 0 || productGroups.length === 0 || rotaGroups.length === 0) {
                ...
                const [{ data: sellersData }, { data: customersData }, { data: groupsData }, { data: rotaData }] = await Promise.all([sellersPromise, customersPromise, groupsPromise, rotaPromise]);
                setSellers(sellersData || []);
                setCustomers(customersData || []);
                setProductGroups(groupsData || []);
                setRotaGroups(rotaData || []);
            }
            ...
            if (filters.groupids.size > 0) {
                const { data: gData } = await supabase
                    ...
                    .select('seller_visit_id')
                    .in('product_group_id', Array.from(filters.groupids))
                    .not('seller_visit_id', 'is', null);
                
                visitIdsByGroups = (gData || []).map(d => d.seller_visit_id);
            ...
            let query = supabase
                .from('seller_visits')
                .select('*, customer:customers(name), seller:sellers(name), visit_type:visit_types(name), group_details:visit_product_group_details(id, before_image_url, after_image_url, notes, product_group:product_groups(name))', { count: 'exact' });
    ...
    }, [pagePermissions, showNotification, sellers.length, customers.length, productGroups.length, rotaGroups.length, currentPage, filters, rotaGroupCustomerIds]);
    ...
    const availableCustomers = useMemo(() => {
        if (!filters.rotaGroupId) return customers;
        return customers.filter(c => rotaGroupCustomerIds.has(c.id));
    }, [customers, filters.rotaGroupId, rotaGroupCustomerIds]);

    if (!pagePermissions?.can_view) return <p>{t('error.accessDenied.message')}</p>;

    return (
        <div className="space-y-6 pb-20">
            <h1 className="text-3xl font-bold">{t('adminSellerVisitReport.title')}</h1>
            ...
            <div className="p-5 bg-surface dark:bg-dark-surface rounded-xl shadow-md border border-border dark:border-dark-border space-y-4">
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div>
                        ...
                        <select value={filters.sellerId} onChange={e => handleFilterChange('sellerId', e.target.value)} className="w-full p-2 bg-surface dark:bg-dark-surface border border-border dark:border-dark-border rounded-md text-sm focus:ring-primary">
                     ...
                     <div>
                         ...
                         <select value={filters.rotaGroupId} onChange={e => handleFilterChange('rotaGroupId', e.target.value)} className="w-full p-2 bg-surface dark:bg-dark-surface border border-border dark:border-dark-border rounded-md text-sm focus:ring-primary">
                             <option value="">{t('visitRequestReport.filters.all')}</option>
...
export default AdminSellerVisitReport;
