...
import { VisitRequest, Customer, RotaGroup } from '../types';
...
const SellerVisitReport: React.FC = () => {
    const { t, permissions, showNotification, profile } = useAppContext();
    const pagePermissions = permissions['Seller Visit Report'];
    ...
    // Fetch initial context (Seller Profile, Associated Customers, Rota Groups)
    useEffect(() => {
        const fetchContext = async () => {
            ...
            try {
                ...
                // Fetch customers assigned to this seller
                const { data: relations, error: relationsError } = await supabase.from('customer_seller_relationships').select('customer_id').eq('seller_id', sellerProfile.id);
                ...
                // Fetch Rota Groups
                const { data: rotaGroupsData, error: rotaGroupsError } = await supabase.from('rota_groups').select('*').order('name');
    ...
    const fetchReports = useCallback(async (page: number, currentFilters: typeof initialFilters) => {
        ...
        try {
            ...
            if (currentFilters.customerId) {
                ...
                query = query.eq('customer_id', currentFilters.customerId);
            } else if (currentFilters.rotaGroupId && rotaGroupCustomerIds.size > 0) {
                // Rota group selected, filter by customers in that group
                query = query.in('customer_id', Array.from(rotaGroupCustomerIds));
            } else if (currentFilters.rotaGroupId && rotaGroupCustomerIds.size === 0) {
                 // Rota group selected but empty
    ...
    }, [pagePermissions, profile, showNotification, t, rotaGroupCustomerIds]);
    
    // Initial fetch
    useEffect(() => {
        fetchReports(1, initialFilters);
    }, [fetchReports]); // Intentionally omitting initialFilters to avoid loop, fetchReports uses current params

    const handleFilterChange = (field: keyof typeof filters, value: any) => {
        setFilters(prev => {
            const newFilters = { ...prev, [field]: value };
            // If changing rota group, reset customer selection to allow filtering
            if (field === 'rotaGroupId') newFilters.customerId = '';
    ...
    // Filter the customer dropdown list based on selected rota group
    ...
    return (
        <div className="space-y-6">
            <h1 className="text-3xl font-bold">{t('sellerVisitReport.title')}</h1>
            ...
            <div className="p-4 bg-surface dark:bg-dark-surface rounded-lg shadow-md border border-border dark:border-dark-border space-y-4">
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                         ...
                         <select value={filters.rotaGroupId} onChange={e => handleFilterChange('rotaGroupId', e.target.value)} className="w-full p-2 bg-surface dark:bg-dark-surface border border-border dark:border-dark-border rounded-md text-sm">
            ...
            {requests.length === 0 ? <p className="text-center py-8 text-text-secondary">{t('sellerVisitReport.noResults')}</p> :
             (
                <>
...
export default SellerVisitReport;
